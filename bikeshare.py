...
### This lab is intended to give you exposure to basic Python, and also to the 
### idea of Monte Carlo simulation, which we'll see a lot. There are 8 questions 
### for you to finish - each is numbered in the comments.

### Suppose that we want to model a bikeshare here at USF. Students will take bikes to the beach, and then
### back to USF. But we're worried that there might not be enough bikes at either 
### end, depending on demand. So we want to simulate some different scenarios and see
### what happens - do all the bikes accumulate at USF? Or maybe at the beach? Or are there
### always bikes at both locations? 
...
# print("Bikes at USF %d" % bikes[1])
## or

# print("Bikes at USF {bikes[1]}")
...
### 3. Make a similar fix to bikeToUSF. 
...
### 4. Let's make a print function that can nicely display the number of bikes in each location.
## This is ok, but uses an older style of printing, which is not as flexible.
## Change it to use f-strings: print(f"this is where {var_name} goes.")
...
def printBikes(bikeList) :
    print(f"Bikes at USF {bikeList[0]}")
    print(f"Bikes at Ocean Beach {bikeList[1]}")
...
### 5. It's confusing and not very flexible to use the threshold variable twice.
...
### Really, what we are interested in is the long-term behavior of the system.
### We do this through Monte Carlo simulation, which is really just
### running the system repeatedly and obsering the outcome.
### To do this, we need iteration.
...
def runSimulation():
    ...
    for i in thresholds :
        for j in thresholds :
            print("i: %f j: %f" % (i,j)) ## note that we're using %f since the values are floating-point numbers.
            bikeSim(100, i,j)
            print(f"After 100 iterations, there are {bikeList[0]} bikes at USF and {bikeList[1]} bikes at Ocean Beach.")


### So what does this tell us? It gives us a way of answering, for different thresholds,
### what we expect the steady state of the system to be. 
### Our next step would be to then rerun this model lots of times to see
### whwether we always get the same outcome for the same input parameters.
